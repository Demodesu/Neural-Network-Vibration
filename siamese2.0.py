...
#         threshold = 200 # define threshold for inversion
#         barrier = lambda x: 0 if x > threshold else 255 # change values of x to 0(black) if more than 255(white)
#         img = img.convert('L').point(barrier,mode='1') # convert to greyscale 
#         img = np.asarray(img)
...
# embedding_network = tf.keras.Model(inputs=input_embedding,outputs=dense,name='embedding')
...
# stop_training = tf.keras.callbacks.EarlyStopping(patience=4)
...
# siamese_model.save_weights("E:\\Gun's stuff\\Machine Learning and Deep Learning\\Machine Learning\\Ajinomoto Project\\Similarity Model Weight 2.0\\Siamese_Weights.h5")
# siamese_model.save("E:\\Gun's stuff\\Machine Learning and Deep Learning\\Machine Learning\\Ajinomoto Project\\Similarity Model 2.0")
...
def img_loader(machine=None):
    ...
    if machine_prediction == True:
        ...
        machine_img = Image.open(machine_img_path)
        threshold = 200 # define threshold for inversion
        barrier = lambda x: 0 if x > threshold else 255
        machine_img = machine_img.convert('L').point(barrier,mode='1')
        ...
        for condition in condition_names:
            ...
            threshold = 200
            barrier = lambda x: 0 if x > threshold else 255
            condition_img = condition_img.convert('L').point(barrier,mode='1')
...
machine_img_list, condition_img_list = img_loader(machine='E3D8')
...
siamese_model.load_weights("E:\\Gun's stuff\\Machine Learning and Deep Learning\\Machine Learning\\Ajinomoto Project\\Similarity Model Weight 2.0\\Siamese_Weights.h5")
